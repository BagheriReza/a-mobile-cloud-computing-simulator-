...
public class HarddriveStorage implements Storage {
	...
	/** Added By Reza, If is equal to "-1" it means datacenter storage
	 * if other number it means local hard disk which belong to host
	 */
	private int HostID;
	private double Energypernuit = 0;
	...
	public int SetHostID(int ID) {
		HostID = ID;
		return HostID;
	}
	
	@Override
	public double getEnergypernuit() {
		return Energypernuit;
	}
	
	public double SetEnergypernuit(double ID) {
		Energypernuit = ID;
		return Energypernuit;
	}
	...
	public double addFile(File file) {
		...
		//Log.printLine("File size: "+file.getSize() +" Current used: "+currentSize+ " Capacity: "+ capacity );
