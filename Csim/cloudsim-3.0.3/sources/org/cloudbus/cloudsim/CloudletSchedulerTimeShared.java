...
public class CloudletSchedulerTimeShared extends CloudletScheduler {
	...
	public double updateVmProcessing(double currentTime, List<Double> mipsShare, int VMID, Datacenter Networkdatacenter) {
		...
		for (ResCloudlet rcl : getCloudletExecList()) {
			...
			// migration condition
			if(MigrationCheck( rcl,VMID, Networkdatacenter)){
				...
				VmExeEnergyDeduction( rcl,VMID, Networkdatacenter);
			...
			if(rcl.getCloudlet().Readmaintain == false){
				Log.printLine("********************************************** Execution clock for maintain: "+ CloudSim.clock());
				Networkdatacenter.IReplicaManagement.Maintain2(Networkdatacenter, CloudSim.clock());
	...
	/**
	 * Reza  
	 * Deducting execution energy
	 */
	private void VmExeEnergyDeduction( ResCloudlet cl,int VMID, Datacenter datacenter){
		double VmEnergy;
		double ExeEnergy;
		double EnergyPerMIPS;
		VmEnergy = datacenter.getVmList().get(VMID).getbattery();
		Log.printLine("VmID: "+VMID+" before deduction: "+ VmEnergy);
		EnergyPerMIPS = datacenter.IVMInfo.ExecEnergyUnit.get(cl.getCloudletId()).get(VMID)/ datacenter.getVmList().get(VMID).getMips();
		Log.printLine("ExePerUnit: "+ datacenter.IVMInfo.ExecEnergyUnit.get(cl.getCloudletId()).get(VMID)+
				"MIPS: "+ datacenter.getVmList().get(VMID).getMips()+ "Time: "+ (cl.getCloudletTotalLength() - cl.getRemainingCloudletLength()));
		ExeEnergy = (cl.getCloudletTotalLength() - cl.getRemainingCloudletLength())*EnergyPerMIPS;
		Log.printLine("CloudletID: "+cl.getCloudletId()+" ,ExeEnergy : "+ ExeEnergy);

		VmEnergy = VmEnergy - ExeEnergy;
		Log.printLine("VmID: "+VMID+" after deduction: "+ VmEnergy);
	 ...
	 */
	private boolean MigrationCheck( ResCloudlet cl,int VMID, Datacenter datacenter){
		double VmEnergy;
		double ExeEnergy;
		double EnergyPerMIPS;
		VmEnergy = datacenter.getVmList().get(VMID).getbattery();
		//Log.printLine("clock: "+CloudSim.clock());
		//Log.printLine("Migration checking, VmID: "+VMID+" orginal Energy: "+ VmEnergy);
		EnergyPerMIPS = datacenter.IVMInfo.ExecEnergyUnit.get(cl.getCloudletId()).get(VMID)/ datacenter.getVmList().get(VMID).getMips();
		ExeEnergy = (cl.getCloudletTotalLength() - cl.getRemainingCloudletLength())*EnergyPerMIPS;
		//Log.printLine("CloudletID: "+cl.getCloudletId()+" ,ExeEnergy : "+ ExeEnergy);

		VmEnergy = VmEnergy - ExeEnergy;
		//Log.printLine("Migration checking,VmID: "+VMID+" after deduction: "+ VmEnergy);
	...
	public double cloudletSubmit(Cloudlet cloudlet, double fileTransferTime) {
		...
		Log.printLine("Cap: "+getCapacity(getCurrentMipsShare())+" , filetransferTime: "+  fileTransferTime+" , length: "+ length);
		Log.printLine("return: "+ cloudlet.getCloudletLength() / getCapacity(getCurrentMipsShare()));
