...
public class AppCloudlet {
	 ...
	 * @param vmSpecification 
	 * @param VMBW 
	 * @param noApp 
	 * @param iVMsTimeExecution 
	 * @param iVMsTimeExecution 
	 * @param IVMInfo 
	 * @param linkDC 
	 * @param  
	 */
	public void createCloudletList(int appcounter, List<Integer> vmIdList, List<NetworkVm> vmSpecification, int[][] VMBW, int noApp, VMsTimeExecution iVMsTimeExecution, VMInfo IVMInfo, NetworkDatacenter linkDC) {
		for (int i = 0; i < numbervm; i++) {
			long length = 4;
			long fileSize = 300;
			long cloudletcharge = 1;
			long outputSize = 300;
			long memory = 256;
			int pesNumber = 4;
