...
public class NetworkHost extends Host {
	...
	public NetworkHost(
		...
		super(id, ramProvisioner, bwProvisioner, storage, Battery, peList, vmScheduler);
	 ...
	 */
	@Override
	public double updateVmsProcessing(double currentTime, Datacenter Networkdatacenter) {
		...
		for (Vm vm : super.getVmList()) {
			double time = ((NetworkVm) vm).updateVmProcessing(currentTime, getVmScheduler()
					.getAllocatedMipsForVm(vm),  Networkdatacenter);
			if (time > 0.0 && time < smallerTime) {
				smallerTime = time;
			}
		}
		// send the packets to other hosts/VMs
		sendpackets( Networkdatacenter);
	...
	private void recvpackets() {
		...
		for (NetworkPacket hs : packetrecieved) {
			...
			/*
				if(vm.getId() == 1){
					Log.printLine("in host received from virtualreciever: "+ hs.pkt.virtualrecvid );
	 ...
	 * 
	 */
	private void sendpackets(Datacenter networkdatacenter) {
		...
		for (Vm vm : super.getVmList()) {
			for (Entry<Integer, List<HostPacket>> es : ((NetworkCloudletSpaceSharedScheduler) vm
				...
				for (HostPacket pkt : pktlist) {
					NetworkPacket hpkt = new NetworkPacket(getId(), pkt, vm.getId(), pkt.sender);//NetworkPacket(getId(), pkt, vm.getId(), pkt.sender)
					//Reza test
					/*
					if(hpkt.pkt.reciever == 1){
						Log.printLine("From host of sender, virtualreciver: "+ hpkt.pkt.virtualrecvid);
						pressAnyKeyToContinue();
						pressAnyKeyToContinue();
					}
		...
		if (flag) {
			for (Vm vm : super.getVmList()) {
				vm.updateVmProcessing(CloudSim.clock(), getVmScheduler().getAllocatedMipsForVm(vm),networkdatacenter);
			}
		}
		...
		for (NetworkPacket hs : packetTosendGlobal) {
			double delay = 0.0;//(1000 * hs.pkt.data) / avband
