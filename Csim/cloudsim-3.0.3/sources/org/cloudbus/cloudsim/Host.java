...
public class Host {
	...
	public double updateVmsProcessing(double currentTime, Datacenter Networkdatacenter) {
		...
		for (Vm vm : getVmList()) {
			double time = vm.updateVmProcessing(currentTime, getVmScheduler().getAllocatedMipsForVm(vm), Networkdatacenter);
	 ...
	 * @param vm the vm
	 */
	public void addMigratingInVm(Vm vm, NetworkDatacenter Networkdatacenter) {
		...
		if (!getVmsMigratingIn().contains(vm)) {
			...
			getVmList().add(vm);
			updateVmsProcessing(CloudSim.clock(),  Networkdatacenter);
			vm.getHost().updateVmsProcessing(CloudSim.clock(), Networkdatacenter);
		}
	}
	...
	public boolean vmCreate(Vm vm) {
		...
		setStorage(getStorage() - vm.getSize());
		setbattery(getbattery() - vm.getbattery());
		getVmList().add(vm);
		vm.setHost(this);
		return true;
	}
