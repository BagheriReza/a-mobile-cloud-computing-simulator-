...
public class CloudSim {
	 ...
	 * @post $none
	 */
	public static double startSimulation(Datacenter datacenter0) throws NullPointerException {
		Log.printLine("Starting CloudSim version " + CLOUDSIM_VERSION_STRING);
		try {
			double clock = run( datacenter0);
	 ...
	 * @return true, if successful otherwise
	 */
	public static boolean runClockTick( Datacenter datacenter0) {
		SimEntity ent;
		boolean queue_empty;
	 ...
	 */
	public static double run( Datacenter datacenter0) {
		if (!running) {
			runStart();
		}
		while (true) {
			if (runClockTick( datacenter0) || abruptTerminate) {
				break;
			}
	 ...
	 * in user simulations.
	 */
	public static void finishSimulation( Datacenter datacenter0) {
